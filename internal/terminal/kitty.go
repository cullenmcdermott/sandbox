...
// displays image id. Each row is a run of U+10EEEE cells: the first cell carries
// the row diacritic and a column diacritic, and the image id is carried in a
// 24-bit foreground color (R=id>>16, G=id>>8, B=id). Rows are separated by '\n'.
// Each cell measures width 1 (the diacritics are zero-width), so a cols-wide row
// occupies exactly cols columns — the same width as the block bar it replaces.
//
