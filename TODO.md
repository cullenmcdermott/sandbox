...
* Investigate "Syncing Files" taking a long time (potentially on slow wifi), even when reconnecting to a previously existing session it seems like its syncing. Can it show progress if nothing else? Can we speed it up? Really reconnecting should feel much faster
