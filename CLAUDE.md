...
| `client/models` (public) | Resolves a model's context-window limit + per-million-token pricing; importable by external SDK consumers. `models.Limit(model)` reports the model's MAXIMUM window and its prices; `models.EffectiveContextLimit(model)` reports the window it actually runs with (Claude's largest windows are an opt-in beta) and is what drives the TUI ctx% indicator |
