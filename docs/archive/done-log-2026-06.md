...
## ~~esc-cancel, opaque modal, yolo-by-default, scroll perf + stick-to-bottom~~ **DONE (2026-06-24)**

Four reported chat-TUI issues, all fixed:

- **esc didn't cancel the running turn.** The TUI called `InterruptTurn` with an
  empty `TurnRef` → URL `/sessions/<id>/turns//interrupt`, which the runner's
  `[^/]+` route never matched, so the SDK query was never aborted. Fixes:
  `TranscriptModel.activeTurnID` captured from `turn.started`
  (`transcript.go` `beginTurn`/`EventTurnStarted`) and passed through
  `interruptTurn`/`queueSteer`; runner route relaxed to `[^/]*` with a
  sole-active-turn fallback (`runner/src/server.ts` interrupt handler); interrupt
  errors now surface a notice (`interruptFailedMsg`) instead of being swallowed.
  Steer (esc with a queued prompt) now defers the new turn to
  `EventTurnInterrupted` so it can't 409 against the still-active turn.
- **Modal backdrop was semi-transparent.** The transcript modal ghosted the
  dashboard (`dimBackdrop`). Now a solid opaque page fill (`app.go` `opaqueBackdrop`).
  The reconnect splash intentionally keeps the dimmed cached conversation
  (`TestConnectingPreviewShowsCachedHistory`).
- **Bypass/yolo exited 1; now the default.** The pod runs the `claude` binary as
  root without `IS_SANDBOX=1`, so the binary's root guard rejected
  `--dangerously-skip-permissions`. Set `IS_SANDBOX=1` in `internal/k8s/backend.go`
  `buildEnv` + the runner spawn env (`runner/src/claude.ts`); also un-breaks the
  always-bypass auto-title summarizer (was silently failing every turn). Default
  permission mode flipped to `modeBypass` (`transcript.go:~427`); `canUseTool`
  omitted under bypass.
- **Scroll lag + flaky stick-to-bottom.** (a) `bodyView` spent ~830µs / 33k
  allocs per frame in one `lipgloss.Style.Width().Height().Render()` → replaced
  with `fitModal` (~600× fewer allocs, byte-identical goldens;
  `transcript_list.go`). (b) `layout()` resized the viewport before reading
  at-bottom, dropping the pin whenever the composer grew / a palette opened. Added
  a durable `follow` flag to `tui/list` (`list.go`) that re-pins on
  SetItems/AppendItems/SetSize and is tracked by all scroll entry points; regression
  tests in `tui/list/list_follow_test.go`, perf guard in `scroll_bench_test.go`.

### Follow-ups / caveats

- **Steer→submit residual race.** `queueSteer` now interrupts then submits on
  `EventTurnInterrupted` (server emits it synchronously on abort), but the runner's
  `finishTurn` (which clears `activeTurns`) runs slightly later as the SDK
  generator winds down. A follow-up `StartTurn` arriving before then still 409s.
  In practice the SSE-out + HTTP-in round trip outlasts the teardown, but the
  fully-correct fix is for the runner to interrupt-and-start atomically (or have
  the interrupt handler await teardown). See `runner/src/server.ts` (interrupt
  handler) + `internal/tui/dashboard/transcript.go` `queueSteer`.
- **Opacity scope.** Only the transcript modal + reconnect splash were made
  opaque. The dashboard-level overlays (`backend_picker.go` `pickerView`,
  `model.go` switcher/permQueue/rename) still composite the live dashboard at Z=0
  — conventional palette behavior. If those should also be opaque, drop a
  `solidBlock(theme.Page)` scrim at Z=0 there.
- **`bodyView` still ~285µs / frame** after the fix — now dominated by `fitModal`'s
  per-line `lipgloss.Width` ANSI scan + `JoinHorizontal` (was the 830µs lipgloss
  block). Further wins possible by having `tui/list` emit width-padded lines or
  caching a running prefix-sum, but the big regression is gone.
