...
// TestWaitExternalReadyAnyStatus: waitExternalReady treats any HTTP response
// (even non-2xx) as ready, because a response at all proves the pod-side server
// answered.
func TestWaitExternalReadyAnyStatus(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusUnauthorized) // 401 still means "listening"
	}))
	defer srv.Close()

	if err := waitExternalReady(context.Background(), srv.URL+"/"); err != nil {
		t.Fatalf("ready probe failed against a live server: %v", err)
	}
}

// TestWaitExternalReadyTransportError: a transport error (nothing listening) is
// retried until the context expires, rather than false-passing the way a bare TCP
// dial did.
func TestWaitExternalReadyTransportError(t *testing.T) {
	...
	if err := waitExternalReady(ctx, url); err == nil {
