...
// diff.go — edit-diff derivation for the transcript reducer. An Edit/Write/
// MultiEdit tool's input carries the before/after text; this turns that into the
// "+"/"−"/" "-prefixed unified diff lines the public chat.ToolItem renders under
// its elbow when expanded (and the chat.Permission card shows for an edit
// approval). Ported behavior-for-behavior from the dashboard's permission_diff.go
// so a public host produces the same diffs without importing internal/.
...
// editDiff extracts a "+"/"−"/" "-prefixed unified diff and the +adds/−dels
// counts from an edit-like tool's input. Non-edit tools (or unparseable input)
// yield nothing.
func editDiff(tool string, input json.RawMessage) (adds, dels int, diffLines []string) {
	switch tool {
	case "Edit":
		var p struct {
...
func condenseDiff(lines []string, maxLines int) []string {
	...
	for i := 0; i < n; {
		...
		if skipped := j - i; skipped > 0 {
			out = append(out, "… "+strconv.Itoa(skipped)+" unchanged")
		}
		i = j
	}
	if len(out) > maxLines {
