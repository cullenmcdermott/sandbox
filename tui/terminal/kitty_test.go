...
func TestKittyPlaceholderWideNoColumnClamp(t *testing.T) {
	// Regression for the 32-entry diacritic-table clamp: a placement WIDER than
	// the table must not collapse columns 32+ onto the clamped last table index.
	// The encoding now writes a column diacritic only on each row's first cell
	// and lets the terminal auto-increment, so for a single row (row 0) every
	// diacritic is index 0 — the clamped last-index mark must never appear, the
	// width must stay exactly cols, and there must be exactly cols cells.
	clampMark := rowColumnDiacritics[len(rowColumnDiacritics)-1]
	for _, cols := range []int{33, 40, 64, 80} {
		s := KittyPlaceholders(7, cols, 1)
		if strings.ContainsRune(s, clampMark) {
			t.Errorf("cols=%d: output contains clamped diacritic %U — columns are clamping", cols, clampMark)
		}
		if w := uniseg.StringWidth(stripSGR(s)); w != cols {
			t.Errorf("cols=%d: width %d, want %d", cols, w, cols)
		}
		if n := strings.Count(s, placeholder); n != cols {
			t.Errorf("cols=%d: %d placeholder cells, want %d", cols, n, cols)
		}
	}
}
